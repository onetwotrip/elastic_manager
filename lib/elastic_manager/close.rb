...
module Close
  ...
  def close_populate_indices(indices, date_from, date_to, daysago)
    ...
    if indices.length == 1 && indices.first == '_all'
      result = @elastic.all_indices(date_from, date_to, daysago, 'open')
    else
      if date_from.nil?
        result = @elastic.all_indices(date_from, date_to, daysago, 'open').select { |r| r.start_with?(*indices) }
      else
  ...
  def do_close(indices)
    indices.each do |index|
      ...
      if index_exist?(response)
        ...
        elastic_action_with_log('close_index', index, @config['settings']['do_not_close_box_type'])
      else
